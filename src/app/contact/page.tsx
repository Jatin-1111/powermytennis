          ...
          <div className="order-2 lg:order-1 flex flex-col h-full gap-8">
            <div className="bg-brand-neutral/10 p-8 rounded-3xl border border-brand-neutral/20 shadow-sm flex flex-col sm:flex-row gap-6 items-center sm:items-start justify-between">
               <div className="text-center sm:text-left">
                 <h3 className="font-black text-brand-primary uppercase tracking-widest text-lg mb-2">Call Us</h3>
                 <p className="text-brand-black text-lg font-medium tracking-wide">{siteConfig.phone}</p>
               </div>
               <div className="flex gap-4">
                 <a href={`tel:+91${formattedPhone}`} aria-label="Call Us" className="w-14 h-14 rounded-full bg-brand-primary text-brand-white flex items-center justify-center hover:bg-brand-secondary transition-colors shadow-md">
                   <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                     <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                   </svg>
                 </a>
                 <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp Us" className="w-14 h-14 rounded-full bg-brand-accent text-brand-primary flex items-center justify-center hover:bg-[#b0c426] transition-colors shadow-md">
            ...
            <div className="bg-brand-neutral/10 p-8 rounded-3xl border border-brand-neutral/20 shadow-sm flex flex-col sm:flex-row gap-6 items-center sm:items-start justify-between">
               <div className="text-center sm:text-left">
                 <h3 className="font-black text-brand-primary uppercase tracking-widest text-lg mb-2">Email Us</h3>
                 <p className="text-brand-black text-lg font-medium tracking-wide">{siteConfig.email}</p>
               </div>
               <div className="flex gap-4">
                 <a href={`mailto:${siteConfig.email}`} aria-label="Email Us" className="w-14 h-14 rounded-full bg-brand-primary text-brand-white flex items-center justify-center hover:bg-brand-secondary transition-colors shadow-md">
                   <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
