        ...
        <MotionSection stagger className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {links.map((link, idx) => (
            <MotionItem key={idx} className="h-full">
              ...
              >
                <h3 className="text-h3 font-black uppercase tracking-wide mb-element">{link.title}</h3>
                <p className="opacity-90 mb-8 font-medium leading-relaxed flex-1">{link.description}</p>
